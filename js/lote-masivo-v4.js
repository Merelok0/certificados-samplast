// SAMPLAST - Carga masiva v4: códigos técnicos, medidas directas, limpieza y PDFs separados.
(() => {
  "use strict";
  ...
  function normCodigo(v) {
    return String(v || "").trim().toUpperCase()
      .replace(/^CODIGO(?:\s+SAP)?\s*[:：]?\s*/i,"")
      .replace(/\s+/g,"");
  }

  function normLote(v) {
    return String(v || "").trim().toUpperCase()
      .replace(/^(?:LOTE|LT)\s*[:：]?\s*/i,"")
      .replace(/\s+/g,"");
  }
  ...
  function detectColor(v) {
    const x = normalizeText(v);
    return [...VALID_COLORS].find(c => new RegExp(`\\b${c}\\b`).test(x)) || "";
  }
  ...
  function isSapCode(codigo) {
    const c = normCodigo(codigo);
    return /^P\d{3,}$/i.test(c) || /^SAP[-_]?[A-Z0-9]+$/i.test(c);
  }
  ...
  function explicitMeasures(raw) {
    const direct = parseFormatoString(raw?.formato_etiqueta ?? raw?.formato ?? raw?.medidas ?? raw?.especificacion ?? raw?.medida ?? "");
  ...
  function decodeCodigo(codigo) {
    ...
    if (!m) return { error: `Código técnico no reconocido: ${c || "vacío"}` };
  ...
  function normalizeRow(raw, source="") {
    ...
    const rawTipo = String(raw?.tipo ?? raw?.categoria ?? raw?.tipo_producto ?? "");
    const originalCodigo = normCodigo(raw?.codigo ?? raw?.codigo_raw ?? raw?.codigo_samplast ?? "");
    const codigoSap = normCodigo(raw?.codigo_sap ?? raw?.sap ?? "");
    const codigoEsSap = isSapCode(originalCodigo);
    const codigo = codigoEsSap ? "" : originalCodigo;
    const lote = normLote(raw?.lote ?? raw?.lote_raw ?? raw?.lt ?? "");
    const tipo = normTipo(rawTipo) || inferTipo(codigo);
    const color = String(raw?.color || detectColor(rawTipo) || "").trim().toUpperCase();
    const explicit = explicitMeasures(raw);

    if (!cliente || !lote) return { cliente, tipo, color, codigo, codigoSap: codigoSap || (codigoEsSap ? originalCodigo : ""), lote, formatoFuente: explicit?.formato || "", error: "Falta CLIENTE o LOTE", source };
    if (!tipo || tipo === "REVISAR") return { cliente, tipo: tipo || "REVISAR", color, codigo, codigoSap: codigoSap || (codigoEsSap ? originalCodigo : ""), lote, formatoFuente: explicit?.formato || "", error: "Falta identificar el TIPO de producto. Revise la etiqueta.", source };
    if (!VALID_TYPES.has(tipo)) return { cliente, tipo, color, codigo, codigoSap: codigoSap || (codigoEsSap ? originalCodigo : ""), lote, formatoFuente: explicit?.formato || "", error: `Tipo no reconocido: ${tipo}`, source };
    if (tipo === "MANUAL_COLOR" && !VALID_COLORS.has(color)) return { cliente, tipo, color, codigo, codigoSap: codigoSap || (codigoEsSap ? originalCodigo : ""), lote, formatoFuente: explicit?.formato || "", error: "MANUAL_COLOR requiere un color válido.", source };
    ...
    if (!measures) {
      if (codigoEsSap || codigoSap) {
        return {
          cliente, tipo, color, codigo:"", codigoSap: codigoSap || originalCodigo, lote, formatoFuente:"",
          error:"La etiqueta solo trae CÓDIGO SAP. Ignore ese código y envíe las medidas impresas en 'formato' (ancho*espesor*peso).",
          source
        };
      }
      if (tipo === "PRE-ESTIRADO") {
        return {
          cliente, tipo, color, codigo, codigoSap:"", lote, formatoFuente:"",
          error:"PRE-ESTIRADO requiere las medidas impresas en 'formato' (ej. 18*09*2.50). El código puede quedar vacío.",
          source
        };
      }
      return {
        cliente, tipo, color, codigo, codigoSap:"", lote, formatoFuente:"",
        error:`${decoded?.error || "Falta código técnico o medidas"}. Si la etiqueta trae medidas directas, colóquelas en 'formato'.`,
        source
      };
    }

    const fechaFab = parseDate(lote);
    if (!fechaFab) return { cliente, tipo, color, codigo, codigoSap: codigoSap || (codigoEsSap ? originalCodigo : ""), lote, formatoFuente: explicit?.formato || "", error: `No se pudo obtener una fecha DDMMAA válida desde ${lote}`, source };
    ...
    return {
      ...
      codigoSap: codigoSap || (codigoEsSap ? originalCodigo : ""),
      lote,
      ancho: measures.ancho,
      espesor: measures.espesor,
      peso: measures.peso,
      formato: measures.formato,
      formatoFuente: explicit?.formato || "",
      fechaFab,
  ...
  function serializeCurrentBatch() {
    const rows = batchItems.map(x => ({
      ...
      color: x.color || "",
      codigo: x.codigo || "",
      formato: x.formatoFuente || ((!x.codigo || x.tipo === "PRE-ESTIRADO") ? (x.formato || "") : ""),
      lote: x.lote || ""
    }));
  ...
  function clearBatch(e) {
    ...
    setButtons();
    status("Carga limpiada. Puede pegar un nuevo lote.");
    bulkInput.focus();
  }
  ...
  function renderTable() {
    ...
    const rows = batchItems.map((x,i) => {
      const codeText = !x.codigo ? "No aplica" : x.codigo;
    ...
    bulkTableContainer.innerHTML = `<table class="bulk-table"><thead><tr><th>#</th><th>Cliente</th><th>Tipo</th><th>Código técnico</th><th>Lote</th><th>Fabricación</th><th>Producto</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>${rows}</tbody></table>`;
  ...
  bulkInput.placeholder = '[{"cliente":"ANTALIS TFM S.A.","tipo":"MANUAL","color":"","codigo":"","formato":"06*20*0.60","lote":"140926/R6-1JC"}]';
  const help = document.querySelector(".bulk-help");
  if (help) help.innerHTML = 'Pegue el JSON generado por la IA. Los códigos SAP internos no se usan; si la etiqueta imprime directamente ancho × espesor × peso, esas medidas deben venir en <strong>formato</strong>.';

  btnLoad.addEventListener("click", loadAll, true);
  btnClear?.addEventListener("click", clearBatch, true);
  btnPrint?.addEventListener("click", printAll, true);
  btnPdf?.addEventListener("click", savePdfsSeparately, true);

  window.SamplastLotesV4 = { parseInput, decodeCodigo, normTipo, inferTipo, parseFormatoString, isSapCode };
})();