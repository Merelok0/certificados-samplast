...
(() => {
  ...
  function normCodigo(v) { return String(v || "").trim().toUpperCase().replace(/^CODIGO\s*[:：]?\s*/i,"").replace(/\s+/g,""); }
  function normLote(v) { return String(v || "").trim().toUpperCase().replace(/^LOTE\s*[:：]?\s*/i,"").replace(/\s+/g,""); }
  function normTipo(v) {
    const x = String(v || "").trim().toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/_/g,"-").replace(/\s+/g," ");
    if (!x) return "";
    if (["MANUAL","STFM","STRETCH FILM MANUAL"].includes(x)) return "MANUAL";
    ...
    if (x === "REVISAR" || x === "REVISAR TIPO" || x === "REVISAR-TIPO") return "REVISAR";
    return x.replace(/ /g,"-");
  }
  function inferTipo(codigo) {
    const c = normCodigo(codigo);
    if (/^\d{4}C\d{3}TD$/i.test(c)) return "MANUAL";
    if (/^\d{4}C\d{4}TG$/i.test(c)) return "AUTOMATICO";
    return "";
  }
  function parseDate(lote) {
    const m = normLote(lote).match(/^(\d{2})(\d{2})(\d{2})(?:\/|$)/);
    if (!m) return "";
    const d=+m[1], mo=+m[2], y=2000 + +m[3], dt=new Date(y,mo-1,d);
    if (dt.getFullYear()!==y || dt.getMonth()!==mo-1 || dt.getDate()!==d) return "";
    return `${y}-${String(mo).padStart(2,"0")}-${String(d).padStart(2,"0")}`;
  }
  function addDay(iso) {
    const [y,m,d]=iso.split("-").map(Number), dt=new Date(y,m-1,d); dt.setDate(dt.getDate()+1);
  ...
  function decode(codigo, tipo) {
    const c=normCodigo(codigo), m=c.match(/^(\d{2})(\d{2})C(\d{3,4})([A-Z][A-Z0-9]*)$/i);
    if (!m) return {error:`Código no reconocido: ${c || "vacío"}`};
    const esp=MICRON_BY_CODE[m[2]];
    if (!esp) return {error:`Código de espesor ${m[2]} fuera de tabla 01-22`};
    if (tipo==="MANUAL" && m[3].length!==3) return {error:`Código MANUAL no coincide con el patrón esperado: ${c}`};
    if (tipo==="AUTOMATICO" && m[3].length!==4) return {error:`Código AUTOMATICO no coincide con el patrón esperado: ${c}`};
    const peso=(+m[3]/10).toFixed(1);
    return {codigo:c,ancho:m[1],espesor:esp,peso,formato:`${m[1]}*${esp}*${peso}`};
  }
  ...
  function parseInput(text) {
    ...
    if (t.startsWith("[") || t.startsWith("{")) {
      const parsed=JSON.parse(t), rows=Array.isArray(parsed)?parsed:(parsed.certificados||parsed.items||parsed.lotes||[]);
  ...
  function status(msg,error=false) {
    if(!bulkStatus) return; bulkStatus.textContent=msg; bulkStatus.classList.toggle("is-error",error); bulkStatus.classList.toggle("is-ok",!!msg&&!error);
  }
  function setButtons() {
    const ok=batchItems.length>0 && batchItems.every(x=>!x.error); if(btnPrint) btnPrint.disabled=!ok; if(btnPdf) btnPdf.disabled=!ok;
  }
  ...
  function loadItem(x) {
    form.cliente.value=x.cliente; form.fecha_fabricacion.value=x.fechaFab; form.fecha_emision.value=x.fechaEmision; form.tipo.value=x.tipo;
    if(form.color) form.color.value=x.color||"";
    if(typeof window.applyTipoRules==="function") window.applyTipoRules();
    form.formato_etiqueta.value=x.formato; form.ancho.value=x.ancho; form.espesor.value=x.espesor; form.peso.value=x.peso; form.lote.value=x.lote; form.lote.dataset.manual="1";
    if(typeof window.applyFormatoToFields==="function") window.applyFormatoToFields();
    if(typeof window.updatePreview==="function") window.updatePreview();
    if(loteDateStatus){loteDateStatus.textContent=`Fecha detectada: ${fmtDate(x.fechaFab)} · Emisión: ${fmtDate(x.fechaEmision)}`;loteDateStatus.classList.remove("is-error");}
  }
  function saveState(){const s={};for(const el of Array.from(form.elements)){if(el.name&&!['button','submit','file'].includes(el.type))s[el.name]=el.value;}s.__manual=form.lote?.dataset.manual||"";return s;}
  function restoreState(s){for(const [n,v] of Object.entries(s)){if(n.startsWith("__"))continue;const el=form.elements.namedItem(n);if(el&&typeof el.value!=="undefined")el.value=v;}if(form.lote)form.lote.dataset.manual=s.__manual||"";if(typeof window.applyTipoRules==="function")window.applyTipoRules();if(typeof window.applyFormatoToFields==="function")window.applyFormatoToFields();if(typeof window.updatePreview==="function")window.updatePreview();}
  const paint=()=>new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));
  async function buildPages(){if(!batchRoot)throw new Error("No se encontró el contenedor de impresión.");if(!batchItems.length||batchItems.some(x=>x.error))throw new Error("Corrija los registros marcados antes de continuar.");const st=saveState();batchRoot.innerHTML="";try{for(let i=0;i<batchItems.length;i++){loadItem(batchItems[i]);await paint();const src=document.getElementById("cert-print-area");if(!src)throw new Error(`No se pudo renderizar el certificado ${i+1}.`);const c=src.cloneNode(true);c.removeAttribute("id");c.classList.add("batch-cert-page");batchRoot.appendChild(c);}}finally{restoreState(st);}return batchRoot;}

  async function printAll(e){e?.preventDefault();e?.stopImmediatePropagation();const old=btnPrint.textContent;try{btnPrint.disabled=true;btnPrint.textContent="Preparando…";status(`Preparando ${batchItems.length} certificados…`);await buildPages();batchRoot.setAttribute("aria-hidden","false");document.body.classList.add("batch-print-mode");await paint();const clean=()=>{document.body.classList.remove("batch-print-mode");batchRoot.setAttribute("aria-hidden","true");btnPrint.disabled=false;btnPrint.textContent=old;};window.addEventListener("afterprint",clean,{once:true});window.print();}catch(err){document.body.classList.remove("batch-print-mode");batchRoot?.setAttribute("aria-hidden","true");btnPrint.disabled=false;btnPrint.textContent=old;status(err.message||"No se pudo imprimir.",true);}}
  ...
  async function pdfAll(e){e?.preventDefault();e?.stopImmediatePropagation();if(typeof html2pdf==="undefined"){status("No se cargó la librería PDF.",true);return;}const old=btnPdf.textContent;try{btnPdf.disabled=true;btnPdf.textContent="Generando…";status(`Generando PDF con ${batchItems.length} páginas…`);const root=await buildPages();document.body.classList.add("batch-pdf-mode");root.setAttribute("aria-hidden","false");await paint();const orientation=form.pdf_orientation?.value==="landscape"?"landscape":"portrait";const blob=await html2pdf().set({margin:0,image:{type:"jpeg",quality:.98},html2canvas:{backgroundColor:"#ffffff",scale:2,logging:false,useCORS:false,allowTaint:false,scrollX:0,scrollY:0},jsPDF:{unit:"mm",format:"a4",orientation},pagebreak:{mode:["css","legacy"]}}).from(root).outputPdf("blob");if(!blob||blob.size<1000)throw new Error("El PDF generado salió vacío.");const n=new Date(),name=`CERTIFICADOS_${String(n.getDate()).padStart(2,"0")}-${String(n.getMonth()+1).padStart(2,"0")}-${String(n.getFullYear()).slice(-2)}_${batchItems.length}.pdf`;download(blob,name);status(`PDF generado: ${name}`);}catch(err){status(`Error al generar PDF: ${err.message||"desconocido"}`,true);}finally{document.body.classList.remove("batch-pdf-mode");batchRoot?.setAttribute("aria-hidden","true");btnPdf.disabled=false;btnPdf.textContent=old;}}
